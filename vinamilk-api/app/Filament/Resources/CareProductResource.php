...
class CareProductResource extends Resource
{
    ...
    public static function productOptionHtml(Product $product): string
    {
        ...
        // First try: volumeMedia main_image
        if ($product->volumeMedia && $product->volumeMedia->first()) {
            $image = $product->volumeMedia->first()->main_image;
        }
        
        // Second try: product main_image
        if (!$image && $product->main_image) {
            $image = $product->main_image;
        }
        
        // Third try: first image from images relationship
        if (!$image && $product->images && $product->images->first()) {
            $image = $product->images->first()->path;
        }
        
        return view('filament.forms.product-select-option', [
            'product' => $product,
            'image' => $image
        ])->render();
    }
