...
class OrderController extends Controller
{
    ...
    public function checkout(Request $request)
    {
        ...
        return DB::transaction(function () use ($request) {
            ...
            // Generate order number early for stock reservation
            ...
            foreach ($request->items as $item) {
                ...
                // Giữ tồn kho (chỉ cho sản phẩm tính tiền)
                if ($purchasePrice > 0) {
                    // Timeout theo phương thức thanh toán
                    $timeoutMinutes = match($request->payment_method) {
                        'cod' => 30,
                        'bank_transfer' => 15,
                        'vnpay', 'momo' => 10,
                        'stripe', 'paypal' => 5,
                    ...
                    // Tính số lượng thực tế cần trừ theo units_per_pack
                    $actualQuantity = $item['quantity'] * ($variant->units_per_pack ?? 1);
            ...
            // Kiện hàng: Mỗi item có package_number riêng theo variant_id
            foreach ($orderItemsData as &$item) {
                if ($item['product_variant_id']) {
                    $item['package_number'] = "{$orderNumber}V{$item['product_variant_id']}";
                } else {
                    $item['package_number'] = "{$orderNumber}GIFT";
                }
            }
            
            // Log order items before creating
            \Log::info("Order items before creating", [
                'order_number' => $orderNumber,
    ...
    {
        // Include care subscription, deliveries and greeting card for care orders so frontend can render per-package schedule
        return Order::with(['items.variant.product', 'payment', 'store', 'careSubscription.deliveries', 'careSubscription.greetingCard'])
    ...
    public function paymentSuccess(Request $request, $orderNumber)
    {
        ...
        try {
            app(StockService::class)->confirmStock($orderNumber);
            \Log::info("Stock confirmed for order: " . $orderNumber);
        } catch (\Exception $e) {
            \Log::error("Failed to confirm stock for order " . $orderNumber . ": " . $e->getMessage());
        }
    ...
    /**
     * Payment failure callback - release reserved stock
     */
    public function paymentFailure(Request $request, $orderNumber)
    {
        \Log::info("paymentFailure endpoint HIT for order: " . $orderNumber);
        ...
        // Hoàn lại kho khi thanh toán thất bại
        try {
            app(StockService::class)->releaseStock($orderNumber);
            \Log::info("Stock released for order: " . $orderNumber);
        } catch (\Exception $e) {
            \Log::error("Failed to release stock for order " . $orderNumber . ": " . $e->getMessage());
        }

        // Update payment log
        $log = \App\Models\PaymentLog::where('order_number', $orderNumber)->first();
        if ($log) {
            $log->update([
                'status' => 'failed',
                'response_payload' => $request->all(),
