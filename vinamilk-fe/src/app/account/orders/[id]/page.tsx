...
export default function OrderDetailPage() {
  ...
  const pkgSubtotals: Record<string, number> = {};
  let overallSubtotal = 0;
  packageKeys.forEach((k) => {
    const items = packages[k] || [];
  ...
  const pkgDiscounts: Record<string, number> = {};
  if (overallSubtotal > 0 && totalDiscount > 0) {
    let allocated = 0;
    const keys = Object.keys(pkgSubtotals);
    keys.forEach((k, i) => {
      const share = pkgSubtotals[k] / overallSubtotal;
      const amt = Math.round(share * totalDiscount);
      pkgDiscounts[k] = amt;
      allocated += amt;
    });
    ...
    if (remainder !== 0) {
      const lastKey = Object.keys(pkgDiscounts).reverse().find(k => pkgSubtotals[k] > 0) || Object.keys(pkgDiscounts)[0];
      if (lastKey) pkgDiscounts[lastKey] = (pkgDiscounts[lastKey] || 0) + remainder;
    }
  } else {
    Object.keys(pkgSubtotals).forEach(k => pkgDiscounts[k] = 0);
  }

  const shippingCost = Number(order.shipping_cost || 0);
  const orderSubtotal = overallSubtotal;
  const orderTotal = Math.max(0, orderSubtotal + shippingCost - totalDiscount);
  const hasOrderDiscount = totalDiscount > 0;
  ...
  return (
    <div className="space-y-6 pb-20 font-sans text-[#002094]">
      ...
      {/* Care Greeting Card */}
      {order.order_type === 'care' && order.care_subscription?.include_greeting_card && order.care_subscription?.greeting_card && (
        <div className="bg-[#ccff33]/10 border border-[#ccff33]/30 rounded-xl p-4">
          <div className="flex items-center gap-4">
            <div className="w-16 h-16 bg-white flex-shrink-0 flex items-center justify-center rounded-lg border border-[#001c9a]/10 overflow-hidden">
              <img 
                src={getImageUrl("gift_default.webp") || ""} 
                alt="Thiệp" 
                className="w-full h-full object-cover" 
              />
            </div>
            <div className="flex-1">
              <p className="text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-1">Thiệp đính kèm</p>
              <p className="text-[15px] font-bold text-[#002094]">{order.care_subscription.greeting_card.title}</p>
              <p className="text-xs text-gray-500 mt-0.5">Thiệp sẽ gửi đến người yêu thương của bạn vào lần đầu tiên giao hàng</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="space-y-6">
        {packages && Object.entries(packages).map(([pkgNum, items]: any, idx) => {
          const isGiftPkg = pkgNum.includes('SXU') || pkgNum.includes('GIFT') || items.some((i: any) => i.is_gift === true || i.price === 0);
          ...
          if (order.order_type === 'care' && order.care_subscription && order.care_subscription.deliveries) {
            const del = order.care_subscription.deliveries.find((d: any) => Number(d.delivery_index) === idx + 1);
            if (del && del.scheduled_date) {
              const date = new Date(del.scheduled_date);
              scheduledDate = date.toLocaleDateString('vi-VN', { day: '2-digit', month: '2-digit', year: 'numeric' });
            }
          }
              ...
              <div className="p-6 space-y-6">
                ...
                >
                    ...
                    <div>
                      ...
                      {scheduledDate && (
                        <div className="text-sm text-gray-500 font-medium">Ngày giao dự kiến: {scheduledDate}</div>
                      )}
                    </div>
                          ...
                          <div className={`space-y-4 ${isGiftPkg ? 'bg-[#F9FFD9] p-4 rounded-lg' : ''}`}>
                            {items.map((item: any) => (
                              <div key={item.id} className="flex gap-4 items-center">
                                <div className="w-14 h-14 rounded flex-shrink-0 flex items-center justify-center p-1 border border-gray-100 relative">
                                  <img
                                    src={getImageUrl(item.image || item.preview_image_path || "gift_default.webp") || "/placeholder.png"}
