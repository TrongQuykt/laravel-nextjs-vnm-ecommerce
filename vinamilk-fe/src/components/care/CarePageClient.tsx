...
import { careApi, catalogApi, getImageUrl } from "@/lib/api";
...
import { Minus, Plus, Trash2, Star, Coffee, Bird, Flower2 } from "lucide-react";
...
export function CarePageClient({ initialSettings, initialProducts = [] }: Props) {
  ...
  return (
    <div className="bg-[#fefef0] min-h-screen">
      ...
      <section className="bg-[#0213b0] text-white pt-28 pb-16">
        <div className="container mx-auto px-6 max-w-6xl">
          {/* Title + tagline row */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-start mb-12 pt-6">
            <div className="pt-12">
            ...
            <div className="col-span-2 space-y-4">
              <h2 className="italic text-xl md:text-2xl !text-white leading-snug">
                &ldquo;{settings?.tagline || "Yêu thương là hiện diện mỗi ngày, bằng mọi cách."}&rdquo;
              </h2>
              <div className="text-sm md:text-base text-white space-y-4">
                <p>
                  Bạn muốn chăm sóc ba mẹ hay người thân nhưng sợ bận rộn rồi quên?<br />
                  Với Vinamilk Care, bạn chỉ cần chọn gói định kỳ 3, 6 hoặc 9 tháng 1 lần duy nhất.
                </p>
                <p>
                  Vinamilk sẽ thay bạn mang sữa đến tận tay người nhận đều đặn mỗi tháng.<br />
          ...
          {/* Benefits card — full image on top, 3 cols text below */}
          <div className="bg-[#fffff1] rounded-2xl overflow-hidden">
            {/* Hero image — full width, no crop/split */}
            {settings?.hero_image_path ? (
              ...
              <div className="w-full h-52 bg-[#d3e1ff]/30 animate-pulse" />
            )}

            {/* 3 benefit columns */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-0 divide-y md:divide-y-0 md:divide-x divide-[#001c9a]/10">
              {benefits.slice(0, 3).map((b, i) => (
                <div key={i} className="p-8">
                  <div className="text-[#001c9a] mb-4">
                    {i === 0 && <Coffee size={28} strokeWidth={2} />}
                    {i === 1 && <Bird size={28} strokeWidth={2} />}
                    {i === 2 && <Flower2 size={28} strokeWidth={2} />}
                  </div>
                  <h3 className="font-black text-[#001c9a] text-lg mb-2">{b.title}</h3>
                  <p className="text-sm text-[#001c9a]/65 leading-relaxed">{b.description}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      <section className={`pb-32 pt-8 ${view !== "main" ? "hidden" : ""}`} id="chon-san-pham">
        <div className="container mx-auto px-6 max-w-6xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {loadingProducts ? (
              Array.from({ length: 3 }).map((_, i) => (
                <div key={i} className="flex flex-col h-full animate-pulse border border-[#001c9a]/10 p-4 rounded-lg bg-white/50">
                  <div className="aspect-square bg-gray-200 mb-4 rounded-lg bg-[#001c9a]/5" />
                  <div className="h-3 bg-[#001c9a]/10 w-1/3 mb-2 rounded" />
                  <div className="h-6 bg-[#001c9a]/10 w-2/3 mb-3 rounded" />
                  <div className="h-10 bg-[#001c9a]/10 w-full mb-4 rounded" />
                  <div className="mt-auto pt-3">
                    <div className="h-10 bg-[#001c9a]/10 w-full mb-4 rounded" />
                    <div className="h-8 bg-[#001c9a]/10 w-full mb-2 rounded" />
                    <div className="h-10 bg-[#001c9a]/10 w-full rounded" />
                  </div>
                </div>
              ))
            ) : products.map((p) => {
      ...
      {view !== "main" && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-[#000000cc] p-4 md:p-6">
          <div className="bg-[#fffff1] w-full max-w-6xl max-h-full md:max-h-[90vh] rounded flex flex-col overflow-hidden relative shadow-2xl border border-[#001c9a]/10">
            ...
            <div className="px-6 py-6 border-b border-[#001c9a]/10 shrink-0">
            ...
            <div className="flex-1 overflow-y-auto navy-scrollbar">
              {view === "package" && <CarePackageStep onContinue={() => router.push("/care/thanh-toan")} />}
            </div>
          </div>
        </div>
      )}
          ...
          <div className="px-6 py-4 flex items-center justify-between gap-4">
            <div className="text-[#001c9a] text-sm md:text-base flex items-center">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="font-black text-lg">{formatVnd(lineTotal)}</span>
              </div>
            </div>
...
function CareProductCard({
  ...
  useEffect(() => {
    ...
    catalogApi.getProduct(product.slug)
      .then(res => { setFullProduct((res.data ?? res.product) as Product); })
      .catch(() => { })
  ...
  return (
    <div className="bg-transparent flex flex-col h-full">
      ...
      <button type="button" onClick={onOpen} className="w-full text-left group">
        <div className="aspect-square flex items-center justify-center mb-4 bg-transparent p-2">
          {product.image
            ...
            : <div className="w-full h-full bg-[#001c9a]/5 rounded-lg animate-pulse" />}
        </div>
      </button>
      ...
      {product.short_description && (
        <div className="text-[13px] text-[#001c9a] line-clamp-2 mb-3 leading-relaxed"
          dangerouslySetInnerHTML={{ __html: product.short_description }} />
      )}

      <div className="mt-auto pt-3">
        {/* Variant + price row — skeleton while loading */}
        {loadingVariant ? (
          <div className="h-10 bg-[#001c9a]/8 rounded animate-pulse mb-4" />
        ) : (
          <div className="flex items-center justify-between gap-2 mb-4 bg-[#f4f7ff] px-3 py-2.5 rounded">
            <div className="text-[12px] font-medium text-[#001c9a] line-clamp-1 flex-1">{variantName}</div>
            <div className="flex items-center gap-2">
        ...
        <div className="flex flex-col gap-2">
          <div className="flex items-center justify-between border border-[#001c9a] rounded bg-transparent text-[#001c9a] px-3 py-1.5">
            <button type="button" onClick={() => onQtyChange(Math.max(1, qty - 1))} className="p-1"><Minus size={16} strokeWidth={1.5} /></button>
            <span className="text-sm font-bold">{qty}</span>
            <button type="button" onClick={() => onQtyChange(Math.min(99, qty + 1))} className="p-1"><Plus size={16} strokeWidth={1.5} /></button>
          </div>
          <button
            type="button"
            onClick={handleAdd}
            disabled={isAdding || loadingVariant}
          ...
          >
            {isAdding
              ? <div className="w-4 h-4 border-2 border-[#001c9a] border-t-transparent rounded-full animate-spin" />
              : 'Thêm vào gói'}
          </button>
        </div>
      </div>
